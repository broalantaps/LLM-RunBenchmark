...
class Model:
    def __init__(self,model_name='gemma-7b-it',use_gpu="gpu",batch_size=1,data_path='dataset/test-v2.0.json',input_max_length=32,output_max_length=64):
        self.model_name = model_name
        self.device = torch.device("cuda:3" if use_gpu=="gpu" else "cpu")
    ...
    def _load_models(self):
        ...
        if self.device.type == "cpu":
            ...
            self.model = AutoModelForCausalLM.from_pretrained(self.model_name,proxies=huggingface_proxies,quantization_config=quantization_config,device_map=self.device)
        else:
    ...
    def test_infer(self):
        ...
        for k in tqdm(range(0,5),desc="processing epochs"):
            ...
            for batch_inputs, batch_masks in tqdm(dataloader,desc=f"Epoch {k+1}",leave=False):
                ...
                outputs = self.model.generate(input_ids=batch_inputs, max_length=self.output_max_length,do_sample=False)
                ...
                for output in outputs:
                    ...
                    if "llama" in self.model_name:
                        ls = torch.sum(output[self.input_max_length:]!=self.tokenizer.unk_token_id)
                        mask = output[self.input_max_length:] != self.tokenizer.unk_token_id 
        ...
        # Remove outliers
        lambda_outlier = lambda lst: sum(sorted(lst)[1:-1]) / (len(lst)-2) if len(lst) > 2 else sum(lst) / len(lst)
        ...
        print("-"*100)
        print(f"Average Time (excluding extremes): {average_time}s")
        print(f"Average Throughput (excluding extremes): {average_throughput:.2f}tokens/s")
        print(f"Average Memory Usage (excluding extremes): {average_mem}MB")
        print(f'Average Inverse Throughput (excluding extremes): {average_inv_throughput}s/token')
        print("-"*100)
        wandb.log({
            "Average Time (excluding extremes)": average_time,
            "Average Throughput (excluding extremes)": average_throughput,
            "Average Memory Usage (excluding extremes)": average_mem,
            "Average Inverse Throughput (excluding extremes)": average_inv_throughput
        })
        wandb.finish()
...
def main():
    # login()
    wandb.login()
    wandb.init(project="Latest_GPU",name='INT8-llama13B-size=1')
    args = parse_args()
    args_dict = vars(args)
    ...
    model = Model(model_name=args.model,use_gpu=args.device,batch_size=args.batch_size,data_path='dataset/test-v2.0.json',
                  input_max_length=args.input_max_length,output_max_length=args.output_max_length)
